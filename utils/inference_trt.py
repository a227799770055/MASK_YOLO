...
if __name__ == '__main__':
    ...
    yolo_backbone_wts = cfg['model']['backbone']
    yolo_backbone = loadTRTmodel(yolo_backbone_wts)
    yolo_head_wts = cfg['model']['headdetector']
    yolo_head = torch.load(yolo_head_wts)
    yolo = yoloModelPack2TRT(yolo_backbone, yolo_head)
    
    #   Model roi_head loading
    mask_featurealign_wts = cfg['maskrcnn']['featurealign']
    mask_featurealign = loadTRTmodel(mask_featurealign_wts)
    mask_roipool_wts = cfg['maskrcnn']['roipool']
    mask_roipool = torch.load(mask_roipool_wts)
    mask_headdetector_wts = cfg['maskrcnn']['headdetector']
    mask_headdetector = loadTRTmodel(mask_headdetector_wts)
    mask_head = maskModelPack2TRT(mask_featurealign, mask_roipool, mask_headdetector)
